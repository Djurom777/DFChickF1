...
final class RemoteConfigManager {
    ...
    private init() {
        ...
        settings.minimumFetchInterval = 0 // для теста, в проде можно поставить 3600
        remoteConfig.configSettings = settings
    }
    
    /// Получаем значение ключа `url_link` из Remote Config
    func fetchURLLink(completion: @escaping (String?) -> Void) {
        remoteConfig.fetchAndActivate { status, error in
            if let error = error {
                print("Ошибка fetch Remote Config: \(error.localizedDescription)")
                completion(nil)
                return
            }
            
            let value = self.remoteConfig["url_link"].stringValue
            completion(value)
        }
    }
}
